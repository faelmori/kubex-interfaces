...
import (
	...
	"github.com/faelmori/kbxutils/utils"
...
func NewConfigWithArgs(dir string, workerCount int, outputFile string) *Config {
...
func (c *Config) Load() error {
	...
	dir, dirErr := utils.SanitizePath(filepath.Dir(c.dir), c.dir)
	if dirErr != nil {
		l.Error("Error sanitizing path", map[string]interface{}{"error": dirErr.Error()})
		return dirErr
	}
	...
	outputFile, dirErr := filepath.Abs(c.outputFile)
	if dirErr != nil {
		l.Error("Error getting absolute path", map[string]interface{}{"error": dirErr.Error()})
		return dirErr
	}
	outputFile, dirErr = utils.SanitizePath(filepath.Dir(outputFile), outputFile)
	if dirErr != nil {
		l.Error("Error sanitizing path", map[string]interface{}{"error": dirErr.Error()})
		return dirErr
	}
	if _, statErr := os.Stat(outputFile); statErr != nil {
		if mkDirErr := os.MkdirAll(outputFile, 0755); mkDirErr != nil {
			l.Error("Error creating output file directory", map[string]interface{}{"error": mkDirErr.Error()})
			return mkDirErr
		}
		//writer := l.SetLogWriter
		//l.SetLogWriter(outputFile)
	}

	l.Notice("Configuration loaded from environment", nil)
	l.Notice("========================================", nil)
	l.Notice("Directory: "+dir, nil)
	l.Notice("Worker Count: "+fmt.Sprintf("%d", c.workerCount), nil)
	l.Notice("Output File: "+c.outputFile, nil)
	l.Notice("Environment CPU Count: "+fmt.Sprintf("%d", c.enviroment.CpuCount()), nil)
	l.Notice("Environment OS: "+c.enviroment.Os(), nil)
	l.Notice("Environment Hostname: "+c.enviroment.Hostname(), nil)
	l.Notice("Environment Mem Total: "+fmt.Sprintf("%d", c.enviroment.MemTotal()), nil)
